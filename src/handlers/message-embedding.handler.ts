...
import { recordMessageEmbedding, touchMessageEmbeddingLastSpokeAt } from '@/helpers/message-embedding';
...
import { isCommandMessage } from '@/utils/command-message';
...
async function handleMessageEmbedding(_client: NapLink, data: OneBotV11.GroupMessageEvent): Promise<void> {
    ...
    const preference = touchMessageEmbeddingLastSpokeAt(db, userId, timestamp);
    if (preference.optedOut || isCommandMessage(data, config.command.prefix)) return;
